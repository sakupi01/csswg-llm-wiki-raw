---
number: 1166
title: "[popover] Imperative Invokers and light dismiss"
state: "open"
labels: ["needs-triage"]
author: "lukewarlow"
created_at: "2025-02-20T16:50:40Z"
closed_at: null
url: "https://github.com/openui/open-ui/issues/1166"
comments: 2
...
<!-- comment id=2672108553 author=mfreed7 created=2025-02-20T16:59:32Z url=https://github.com/openui/open-ui/issues/1166#issuecomment-2672108553 -->
## @mfreed7 — 2025-02-20

No, that feels broken to me. Indeed, it's because [popover target element](https://html.spec.whatwg.org/multipage/popover.html#popover-target-element) doesn't look at imperative invokers, only the `popovertarget` attribute. It should look at both.

<!-- comment id=2699069768 author=lukewarlow created=2025-03-04T22:14:52Z url=https://github.com/openui/open-ui/issues/1166#issuecomment-2699069768 -->
## @lukewarlow — 2025-03-04

```html
<!DOCTYPE html>
<meta charset="utf-8" />
<title>Popover light dismiss behavior with imperative invocation</title>
...
<button id=b1t>Popover 1</button>
<button id=b1s>Popover 1</button>
<span id=outside>Outside all popovers</span>
<div popover id=p1>
  <span id=inside1>Inside popover 1</span>
  <button id=b2>Popover 2</button>
...
<script>
  ...
  promise_test(async () => {
    popover1.showPopover({source: button1show});
  ...
  promise_test(async () => {
    ...
    popover2.showPopover({source: button2});
...
<script>
  ...
  promise_test(async () => {
    p3.togglePopover({source: button3});
...
<script>
  ...
  promise_test(async () => {
    popover7.showPopover({source: button7});
...
promise_test(async () => {
  ...
  convoluted_p2.togglePopover({source: convPopover1.querySelector('button')});
  assert_true(convPopover1.matches(':popover-open'));
  assert_true(convPopover2.matches(':popover-open'));
  convoluted_p3.togglePopover({source: convPopover2.querySelector('button')});
  assert_true(convPopover1.matches(':popover-open'));
  assert_true(convPopover2.matches(':popover-open'));
  assert_true(convPopover3.matches(':popover-open'));
  convoluted_p4.togglePopover({source: convPopover3.querySelector('button')});
  assert_true(convPopover1.matches(':popover-open'));
  assert_true(convPopover2.matches(':popover-open'));
  assert_true(convPopover3.matches(':popover-open'));
...
promise_test(async () => {
  convPopover1.showPopover(); // Programmatically open p1
  convPopover2.togglePopover({source: convPopover1.querySelector('button')});
  assert_true(convPopover1.matches(':popover-open'));
  assert_true(convPopover2.matches(':popover-open'));
  assert_false(convPopover3.matches(':popover-open'));
  assert_false(convPopover4.matches(':popover-open'));
...
<button id=b29>Open popover 29</button>
...
I think this is a WPT to test this. Will need a spec change too, which I'll look at making.
