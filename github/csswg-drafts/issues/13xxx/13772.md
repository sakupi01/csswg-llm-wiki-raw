...
<!-- comment id=5581083412 author=ExtAnimal created=2026-09-08T07:31:08Z url=https://github.com/w3c/csswg-drafts/issues/13772#issuecomment-5581083412 -->
## @ExtAnimal — 2026-09-08

Are any of the browser vendors making any progress on this?

Because I suspect that universally, applications which need to clear a state or take some other action when a scroll is finished use a timer which kicks off on every `scroll` event, cancelling the last timer from the last `scroll` event if any.

This is incredibly wasteful.

But the `scrollend` event MUST actually work, and fire when a scroll has completely finished for this pattern to stop being used. Otherwise some states will be locked in place.

Even when they all pull their fingers out (And Safari probably will not for another few years), after *all* browsers have fixed this, event *even then* we here will not be able to roll out a better, more efficient way of responding to scrollend for another two years after that (Our legacy browser support policy allows use of browsers up to 2 years old)

I might be retired before that happens. I will email my old colleagues to remind them about this issue!

<!-- comment id=5581242454 author=bramus created=2026-09-08T07:44:13Z url=https://github.com/w3c/csswg-drafts/issues/13772#issuecomment-5581242454 -->
## @bramus — 2026-09-08

@ExtAnimal From Chromium’s side there is https://issues.chromium.org/issues/510319892 that you can track. Hit the +1 in the top right to signal that you are impacted by this, or hit the star (next to the title of the bug) to subscribe to updates. The bug is currently assigned, which is already quite a good thing :)
