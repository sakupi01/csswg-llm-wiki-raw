...
<!-- comment id=5777776043 author=cdoublev created=2026-09-22T13:53:21Z url=https://github.com/w3c/csswg-drafts/issues/13917#issuecomment-5777776043 -->
## @cdoublev — 2026-09-22

Also, when the UA does not support any of the page's supported color schemes in `support`, the algo returns the UA default color scheme without considering the user `preference`. I think it should return `preference` if it is not "no preference".

  > 1. If `support` is null, or none of the [color schemes](https://drafts.csswg.org/css-color-adjust-1/#color-scheme) in `support` are supported by the UA, return the UA default color scheme, and annotate that this color scheme was [defaulted](https://drafts.csswg.org/css-color-adjust-1/#color-scheme-default).
