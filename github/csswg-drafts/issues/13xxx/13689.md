...
<!-- comment id=5087556360 author=kbabbitt created=2026-07-27T05:20:55Z url=https://github.com/w3c/csswg-drafts/issues/13689#issuecomment-5087556360 -->
## @kbabbitt — 2026-07-27

When I presented this proposal at the April F2F, a couple of people brought up `::nth-word()` as an alternate construct for these scenarios. I was resistant at the time due to concerns about the performance of styling each word. But since then, I've prototyped `::nth-word()` and found it to be comparable in performance to the Text Animations prototype I had built previously. Which makes sense: The cost to style each word via `::nth-word()` should not be any worse than the cost to style each word via a wrapping `<span>`, and in Blink at least, pseudo-elements get selector-matched at the same time as their base elements so that we can discover what pseudos we need to instantiate, and `::nth-word()` benefits from that optimization. So that concern has gone away.

`::nth-letter()` and `::nth-word()` were already being discussed in #3208, so I will continue there.
