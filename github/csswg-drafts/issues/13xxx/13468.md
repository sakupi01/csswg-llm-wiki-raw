...
state: "open"
labels: ["Agenda+", "css-anchor-position-1", "css-position-3"]
author: "bfgeek"
...
<!-- comment id=5443109897 author=bfgeek created=2026-08-27T17:56:13Z url=https://github.com/w3c/csswg-drafts/issues/13468#issuecomment-5443109897 -->
## @bfgeek — 2026-08-27

I was looking at implementing this issue, but there is a unspecified side-effect wrt. the top layer.

1. Top Layer elements **have to** be laid out after all the all the other out-of-flow positioned elements have been processed. (Other out-of-flow positioned elements can insert/remove top layer elements as part of layout).

2. Top Layer elements can be **either** `position:fixed` or `position: absolute`. See below[1] for an example of a `position: absolute` element.
...
dialog#target {
  ...
  inset: auto;
  top: anchor(bottom --fixed);
  left: anchor(right --fixed);
}
dialog#target::backdrop {
  display: none;
}
#fixedpos {
  background: lime;
  ...
  anchor-name: --fixed;
  width: anchor-size(--absolute, 100px);
  top: anchor(bottom --absolute);
  left: anchor(right --absolute);
}
...
<body style="height: 200vh; border: solid 2px;">
  <div id="fixedpos">
    <div>fixed-pos</div>
...
Given the constraints, I think this means that a `position: absolute` in the Top Layer, has to be laid out within the [fixed positioning containing block](https://drafts.csswg.org/css-position-3/#fixed-positioning-containing-block), but with modifications (that position attachment is disabled or something?).

Or we do something else.

We can't go backwards and layout a Top-Layer `position:absolute` within the position-absolute containing-block.

cc/ @tabatkins 

[1] `position: absolute` in top-layer:
```
<!DOCTYPE html>
<style>
dialog#target {
