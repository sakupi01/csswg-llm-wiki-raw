...
state: "open"
labels: ["Needs Edits", "css-scoping-1", "css-scoping-2", "css-shadow-1", "selectors-4", "selectors-5"]
author: "LeaVerou"
...
<!-- comment id=2622719814 author=css-meeting-bot created=2025-01-29T20:01:37Z url=https://github.com/w3c/csswg-drafts/issues/11000#issuecomment-2622719814 resolution=true -->
...
<!-- comment id=5645216542 author=cdoublev created=2026-09-12T10:04:15Z url=https://github.com/w3c/csswg-drafts/issues/11000#issuecomment-5645216542 -->
## @cdoublev — 2026-09-12
...
Currently (a year and a half later), it seems that `:host { & {} }` matches but not `:scope { & {} }` nor even just `:scope`.

Indeed, this [WPT test](https://wpt.fyi/results/css/css-shadow/scope-pseudo-in-shadow.html?label=master&label=experimental&aligned&q=css%2Fcss-shadow%2Fscope-pseudo-in-shadow.html) expects `:scope` to match nothing in a shadow root, and all browsers pass it successfully.

93e7cbc added this change:
...
<details>
<summary>Aside: I think it would be useful to clarify (in the context above, in Selectors, or in CSS in general, I do not know) that <i>root</i> and <i>tree</i> refer to <i>elements</i> rather than <i>nodes</i>, or to be more explicit with <a href="https://drafts.csswg.org/css-display-4/#root-element" target="_blank"><i>root element</i></a>, otherwise this is ambiguous. (edit) Moreover, this is incorrect.</summary>
<br>
...
doc.querySelector(':scope *') // null
document.querySelector(':scope *') // <head>
```

</details>

So this makes me wonder if this issue still has "Needs edits" for [CSS Shadow](https://drafts.csswg.org/css-shadow-1/#host-element-in-tree):

  > Only the `:host`, `:host()`, and `:host-context()` pseudo-classes are allowed to match [the shadow host].

Or if "`:scope` resolves to `:host` in a shadow tree" needs to be stated more explicitly in Selectors.

Or if this WPT test simply needs to be updated.

Or if browsers simply disagree with this change.
