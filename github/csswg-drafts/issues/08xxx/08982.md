...
<!-- comment id=5546956546 author=WebReflection created=2026-09-04T22:03:22Z url=https://github.com/w3c/csswg-drafts/issues/8982#issuecomment-5546956546 -->
## @WebReflection — 2026-09-04

imho worklets feel like the best all-in-one answer around computed and whatnot, my only concern is possible future name clashing so that like vars have `--` prefix, worklets should have either `$` (why not?) or some other prefix that helps IDEs, syntax highlights, and future name clashing shenanigans to ever happen, happy to learn this proposal / issue / feature is still discussed.

<!-- comment id=5604857058 author=trusktr created=2026-09-09T16:00:36Z url=https://github.com/w3c/csswg-drafts/issues/8982#issuecomment-5604857058 -->
## @trusktr — 2026-09-09

What do worklets compute? Style values only for specific properties, or only paint pixels with canvas 2D? 

Folks need access to DOM on style change, for example for custom layouts, custom WebGPU rendering based on custom layouts, or etc. Logic needs to read or update DOM references after computed style change, but Worklets don't have access to DOM, so that would be a limiting alternative, unless we first update browsers to allow workers to somehow access DOM from a separate thread.

<!-- comment id=5605829747 author=WebReflection created=2026-09-09T17:12:53Z url=https://github.com/w3c/csswg-drafts/issues/8982#issuecomment-5605829747 -->
## @WebReflection — 2026-09-09

> unless we first update browsers to allow workers to somehow access DOM from a separate thread.

FWIWI we do that via WASM runtimes interacting with JS that is proxied via SharedArrayBuffer to the main thread and resolves synchronously DOM access (or anything else, really) ... if only we could remove the SharedArrayBuffer dependency and have direct DOM access from workers life would be likely better/easier.

**edit** also a bit off topic though, imho
