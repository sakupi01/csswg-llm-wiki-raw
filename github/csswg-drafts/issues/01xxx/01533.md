...
<!-- comment id=5358952313 author=oleastre created=2026-08-20T16:45:31Z url=https://github.com/w3c/csswg-drafts/issues/1533#issuecomment-5358952313 -->
## @oleastre — 2026-08-20

I have a use case for a `:dirty` like selector: change buttons appearance based on form state.
In my use case, I have a wizard like behaviour where a user has to go through several pages, validate or modify input and got to the next page.
Current implementation uses javascript to catch "change" events, but I could imagine having something like:
```html
<form>
  <input name="test" value="Current">
...
form:has(input:dirty) {
  button { display: block }
  a {display: none}
}
```
=> when the user edits a value, the "Save" button appears otherwise, the user gets a link to the next page.
