...
<!-- comment id=5332400117 author=nwidynski created=2026-08-18T18:30:23Z url=https://github.com/w3c/csswg-drafts/issues/1562#issuecomment-5332400117 -->
## @nwidynski — 2026-08-18

@mustaqahmed I've got two more questions that have come up for me while working with the API:

A) Whether a scroll promise is guaranteed to resolve before a `scrollend` event. The spec only defines that there must be no more pending updates, which is identical to `scrollend` as long as the gesture has completed. Would it make sense to define an order?

B) In virtualized scroll containers with dynamic item size, a change to scroll area during scroll currently causes the scroll promise to be interrupted. This is regardless of whether or not `overflow-anchor` is set to `auto`/`none`. I'm not entirely sure if this is by design or an issue with Blinks implementation, but I feel like that shouldn't be the case.

<!-- comment id=5333848066 author=mustaqahmed created=2026-08-18T20:45:00Z url=https://github.com/w3c/csswg-drafts/issues/1562#issuecomment-5333848066 -->
## @mustaqahmed — 2026-08-18

> A) Whether a scroll promise is guaranteed to resolve before a `scrollend` event. The spec only defines that there must be no more pending updates, which is identical to `scrollend` as long as the gesture has completed. Would it make sense to define an order?

The order is mentioned in CSSOM View 1 [perform a scroll](https://drafts.csswg.org/cssom-view/#perform-a-scroll) algorithm. Note two important differences from "scrollend" events:
- If there is no change in scroll position, the event is not fired in Step 7.1 but the promise is still resolved in Step 7.2.
- The promise resolved in Step 7.2 is not necessarily the promise we see from JS. If multiple scrollers are affected by a `scrollIntoView` call, each of those scrollers will have a promise in Step 7.2 that remains hidden from JS, and the promise returned by the call will resolve when all those hidden promises are resolved.

> B) In virtualized scroll containers with dynamic item size, a change to scroll area during scroll currently causes the scroll promise to be interrupted. This is regardless of whether or not `overflow-anchor` is set to `auto`/`none`. I'm not entirely sure if this is by design or an issue with Blinks implementation, but I feel like that shouldn't be the case.

Please make a minimal repro and file a new issue to discuss this. I didn't see any interruption when I locally added new content to [this scroller](https://codepen.io/mustaqahmed/full/gbpmNOP) after a button click!

EDIT: added a link that I missed in the last para.
