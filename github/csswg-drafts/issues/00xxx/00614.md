...
I am very aware of this. See my [Size Calculator](https://sizecalc.hex.xyz/) project which is intended for calculating these relationships.
...
<!-- comment id=5286185959 author=kizu created=2026-08-13T20:45:11Z url=https://github.com/w3c/csswg-drafts/issues/614#issuecomment-5286185959 -->
## @kizu — 2026-08-13

This was brought up in a recent thread in Mastodon: https://front-end.social/@sarajw/117088097081094082

One idea that I wonder if we could re-evaluate — what if this was an `env()` (like `env(pixels-in-cm)` etc.) that browsers could make available in certain conditions (print media, installed apps/web views (although, maybe not by default?), etc). Basically, anywhere browsers could potentially provide the actual size with it being reliable, not adding to the fingerprinting surface, and not having the potential accessibility considerations (not scaling with zoom).

It being an `env()` could allow using it with a fallback, so it could potentially be used easily in general without breaking.
