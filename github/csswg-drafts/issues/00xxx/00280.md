...
<!-- comment id=5587290073 author=surahotoke created=2026-09-08T15:05:33Z url=https://github.com/w3c/csswg-drafts/issues/280#issuecomment-5587290073 -->
## @surahotoke — 2026-09-08

I'd like to add two things to this thread: a new data point since 2016, and a possible way to scope the open questions.

**New data point: a major design system now specifies motion as springs**

Material 3 Expressive (Google, 2025) defines its motion tokens as spring parameters (damping ratio + stiffness) rather than easing curves + durations. Its motion scheme distinguishes "spatial" springs (position/size/shape, may overshoot) from "effects" springs (opacity/color, critically damped). Android/Compose implements this natively.

On the web, the only way to express these tokens today is to pre-sample the spring into `linear()`. I tried to keep the spec values in CSS via `@function`:

```css
@function --spring(--k, --z) {
  result: linear(0, --spring-pt(var(--k), var(--z), 0.0625), /* ...15 more samples... */ 1);
}
```

This works in current Chrome (`sqrt()`, `exp()`, `cos()`, `sin()` are all available inside `@function`, and `linear()` accepts the results), but without any form of repetition in CSS, the number of samples equals the number of hand-written terms. 16 samples is already unwieldy; a lower damping ratio needs more. So `linear()` is a good container for arbitrary curves, but it doesn't make spring *parameters* authorable in CSS. Design tokens like "damping 0.6, stiffness 800" still have to be converted outside the stylesheet.

**Scoping the open questions**

@flackr raised the keyframe/distance problems. Most of them seem to disappear if `spring()` is initially limited to transitions (two endpoints) rather than keyframed animations:

- Distance is simply progress 0→1, so it's defined for opacity, color, clip-path, etc. alike.
- "Settled" can be defined as the progress amplitude falling below a fixed threshold (e.g. 0.1%).
- Instead of `duration: auto`, the spec could say that `transition-duration` is ignored when the timing function is a spring — a precedence rule rather than a new keyword.
- Velocity carry-over: transitions already define behaviour when a running transition is retargeted; a spring variant would add "start the new spring from the current progress and velocity." This addresses @rihok's point without needing a general physics system.

This narrower scope covers what shipped design systems actually use springs for (component state changes), and leaves keyframe integration for later.

Happy to expand on any of this if useful.
