...
<!-- comment id=5481403045 author=graouts created=2026-08-31T16:38:05Z url=https://github.com/w3c/csswg-drafts/issues/9256#issuecomment-5481403045 -->
## @graouts — 2026-08-31

Would be good to get a spec update with this because WPT tests are already being updated based on that but it's not entirely clear to me how this is supposed to work in practice. For instance, the [Web Animations spec says](https://drafts.csswg.org/web-animations-1/#timelines):

> A timeline is considered to be inactive when its time value is unresolved, and active otherwise.

… and [also says](https://drafts.csswg.org/web-animations-1/#waiting-for-the-associated-effect):
...
… just as the [Scroll-driven Animations spec says](https://drafts.csswg.org/scroll-animations-1/#scroll-timeline-progress):

> If the 0% position and 100% position coincide (i.e. the denominator in the [current time](https://drafts.csswg.org/web-animations-1/#timeline-current-time) formula is zero), the timeline is [inactive](https://drafts.csswg.org/web-animations-1/#inactive-timeline).

Just what should we report as `currentTime` for elements without overflow is not obvious to me and a lot falls out of that, just like whether `animation.startTime` would be unresolved even though its `ready` promise would have been fulfilled. This is unfortunate because some of the Interop 2026 tests rely on this change and it's not clear to me how to move forward with those changes without clear spec changes first.

Cc @flackr @birtles @dshin-moz @BorisChiou @kevers-google.

<!-- comment id=5482802306 author=dshin-moz created=2026-08-31T18:37:53Z url=https://github.com/w3c/csswg-drafts/issues/9256#issuecomment-5482802306 -->
## @dshin-moz — 2026-08-31

Right, yeah, spec wording is currently specific about animation being in effect, and what Gecko has (Being in a playing state with unresolved start and current time) doesn't fit that definition. Though, FWIW, I think there's a good reason to have unresolved non-monotonic timelines to be special cased, like @kevers-google alluded to [in this comment](https://github.com/web-platform-tests/wpt/pull/61776/changes/BASE..03c3d46a159cc127435cac026bdf85469c3a65f9#r3894651775).

My understanding is that we want to treat all non-monotonic timelines whose lookup failed as unresolved (or deferred) timelines - They may or may not become valid, and currently are "playing to no effect." I think this special casing is worth it because scrollable overflow may depend on users as much as authors (Monitor size, resolution, window size, etc). Having `getAnimations()` and/or `ready` behave differently in that case feels... fragile (?).
