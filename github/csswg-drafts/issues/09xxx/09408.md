...
<!-- comment id=5727734142 author=woody-li created=2026-09-18T09:03:11Z url=https://github.com/w3c/csswg-drafts/issues/9408#issuecomment-5727734142 -->
## @woody-li — 2026-09-18

Sorry to border, I has a different case about it and not sure whether to create a new issue.
...
<x-test>
  #shadow-dom
    <slot />
  #light-dom
    <button style="anchor-name: --button-box" />
<x-test>
```

Reference failed if set position-anchor in shadow dom styles:
```css
::slotted(button)::after{
    content: '';
    display: block;
    position: absolute;
    position-anchor: --button-box;
...
But works well with set in light dom:
```css
button::after{
    position-anchor: --button-box;
}
...
But the `::slotted(button)` also matches the light dom element, why the difference?
