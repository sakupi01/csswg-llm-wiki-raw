...
Though if such a registry is not possible for some reason and we have to base the checks on IDL, the syntax should be made more CSSy and also _not_ include the abbreviation "IDL" as that's rather a spec term than an author-facing one. It could actually still be mapped to what @fantasai and I proposed earlier, because all IDLs follow a specific naming syntax. The names have the type as prefix, followed by their name, followed by the suffix "Element". And the IDLs also define the attributes. So you could internally translate `html(input[switch])` to `HTMLInputElement.prototype.switch` or `svg(clipPath)` to `SVGClipPathElement`.
...
_That_ is my point. To allow CSS to detect HTML features, the engines need to have some kind of registry it can use and check against. Additional standardization across user agents doesn't seem to be necessary, engines only need to ensure their registries are consistent and kept up-to-date.
...
<!-- comment id=5449151628 author=bramus created=2026-08-28T06:17:52Z url=https://github.com/w3c/csswg-drafts/issues/9746#issuecomment-5449151628 -->
## @bramus — 2026-08-28

In https://bsky.app/profile/tfedder.de/post/3mu3t7yxqzs2g a new use-case was mentioned: checking for support for the `request-close` invoker command. IUC `html()` would solve this use-case: `html(button[command="request-close"])`
