...
<!-- comment id=5559516239 author=RobMayer created=2026-09-06T13:25:18Z url=https://github.com/w3c/csswg-drafts/issues/3524#issuecomment-5559516239 -->
## @RobMayer — 2026-09-06

Bumping with some implementer interest.

I filed a Chromium bug asking for :nth(An+B of A): [https://issues.chromium.org/issues/385595014]. The response:

> It has not been considered yet, but seems like a useful addition. We could implement this if it moves into the css-page spec, and we should also catch up with related development in other specs. For instance, we have :nth-child(), but ALSO :nth-last-child(). So we should probably have an :nth-last() selector as well. It's very common to omit headers and footers (page number, chapter name, whatever) on the last page in every chapter.

The specificity question from the opening post still looks unanswered — were :nth(5) as (0,1,0) and :nth(5 of A) as (1,1,0) ever officially accepted? It bites in book work, where chapter openers fall on rectos, so :nth(1 of chapter) and chapter:right collide on every chapter opening.

Two incidental things: this is still assigned to dauwhe, who seems to be a former editor. And section 3.2 carries no inline issue annotation, so the section reads as settled to anyone not following the tracker.
