...
<!-- comment id=5213853990 author=md84419 created=2026-08-07T07:23:54Z url=https://github.com/w3c/csswg-drafts/issues/3377#issuecomment-5213853990 -->
## @md84419 — 2026-08-07

- Browser-based TVs, Opera, Firefox, Chrome and Edge have had spatial navigation support for ages.
- Opera and Firefox support it by default as part of Blink.
- In Chrome and Edge it can be turned on by adding the feature switch --enable-spatial-navigation to chrome.exe or edge.exe on startup. There doesn't appear to be a way to turn it on or off via settings.
- Embedded Safari (e.g. Apple TV) supports spatial navigation as does WebKitGTK, Webkit Embedded Ports and WebKit EFL port.
- Safari for MacOSX supported it up until Safari 9 (circa 2015) when Webkit forked into four versions of Webkit: the three verisions mentioned above and WebKit for desktop browsers (Safari for MacOSX).  The former continue to support Spatial Navigation whilst the latter currently does not.

The abandoned CSS-nav-1 proposed standard includes a polyfill implementation

- [Spec](https://drafts.csswg.org/css-nav-1/) 
- [Demo1](https://wicg.github.io/spatial-navigation/demo/) - use the arrow keys to navigate
- [Polyfill](https://wicg.github.io/spatial-navigation/polyfill/spatial-navigation-polyfill.js)

From a user perspective, it just works. Technically, there are some obscure pathological edge cases under which it impossible to navigate to every element but these are exceedingly rare in real life. A spatial navigation model which is enabled by default and works for the real world would be a significant improvement over the status quo.
