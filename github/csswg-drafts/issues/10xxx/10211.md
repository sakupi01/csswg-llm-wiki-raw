...
Edit: I no longer think the above edit is the right direction.

<!-- comment id=4927019754 author=romainmenke created=2026-07-09T15:52:36Z url=https://github.com/w3c/csswg-drafts/issues/10211#issuecomment-4927019754 -->
## @romainmenke — 2026-07-09

@LeaVerou @svgeesus I am unsure if the last resolution in this thread has the intended effect.
Also see: https://github.com/w3c/csswg-drafts/issues/14100

When implementing this the outcome is that powerless `hue` values are always converted to `none`.
For example: `lch(from lch(20 0 180) l c h)` has a `none` hue.

Can you maybe provide examples of what the original problem was and the intended outcome for those?

<!-- comment id=5020468348 author=romainmenke created=2026-07-20T08:59:12Z url=https://github.com/w3c/csswg-drafts/issues/10211#issuecomment-5020468348 -->
## @romainmenke — 2026-07-20

https://github.com/w3c/csswg-drafts/commit/226658f3d76cf165495b8faa92e8dfe60f7d7ae6 Removed the only reference to [`prepare a color for conversion`](https://drafts.csswg.org/css-color-4/#prepare-a-color-col1-for-conversion).

This makes it even less clear what the purpose was of the last resolution :)

It would really help to have some examples of the original problem and how the updated spec text affects those cases. These could also be used to update the WPT tests.

<!-- comment id=5178588261 author=LeaVerou created=2026-08-04T11:50:32Z url=https://github.com/w3c/csswg-drafts/issues/10211#issuecomment-5178588261 -->
## @LeaVerou — 2026-08-04

Another one, that UAs are remarkably consistent in getting it wrong: https://codepen.io/leaverou/pen/XJpybdZ?editors=1100

There is absolutely no reason to convert `none` to `0` in `oklch(from var(--some-oklch-color) l c h)` no matter how many levels of nesting you have!

<!-- comment id=5178669358 author=romainmenke created=2026-08-04T11:57:47Z url=https://github.com/w3c/csswg-drafts/issues/10211#issuecomment-5178669358 -->
## @romainmenke — 2026-08-04

Yes UA's haven't caught up yet :)
That is already well established.

I have a lot of tests prepared that I would like to  submit in WPT:
- For `none`: https://github.com/csstools/postcss-plugins/blob/main/packages/css-color-parser/test/basic/none-exhaustive.mjs
- For powerless: https://github.com/csstools/postcss-plugins/blob/main/packages/css-color-parser/test/basic/powerless-exhaustive.mjs

These are very exhaustive instead of only testing a minimal amount and depending on implementers to infer all other cases.

But a blocker for this is this issue :)
Because we have a resolution that doesn't actually have a defined problem.
The last resolution in this thread doesn't actually have examples of problems or what the expected outcome is instead. So I am unsure if this resolution is merely reinforcing what was already in the spec (i.e. carry forward `none` and don't eagerly convert `none` to `0` in RCS). Or if maybe it is describing a novel aspect for cases that I haven't covered yet in the tests above.

I really want to avoid submitting a large amount of tests only to later find out that these are also incorrect/incomplete.

----

Aside from that the resolution itself is a bit wonky.
It split something into two parts.
But nothing (as far as I call tell; see: https://github.com/w3c/csswg-drafts/issues/10211#issuecomment-5020468348) only uses one of those parts, both are always required.

<!-- comment id=5202390542 author=css-meeting-bot created=2026-08-06T08:42:11Z url=https://github.com/w3c/csswg-drafts/issues/10211#issuecomment-5202390542 -->
## @css-meeting-bot — 2026-08-06

The CSS Working Group just discussed ``[css-color-4] Clarify that `none` is preserved in calculations``.

<details><summary>The full IRC log of that discussion</summary>
&lt;TabAtkins> romain: The spec is fairly clear here, but we have a reso in the issue, but the thread doesn't give examples of what it's trying to solve, which makes it hard to come up with tests<br>
&lt;TabAtkins> romain: i've been adding more WPTs, but it's unclear if my impl is doing the right thing<br>
&lt;lea> q+<br>
&lt;astearns> ack lea<br>
&lt;TabAtkins> romain: it's possible this resolutionw as only adding clarification to existing parts of the spec and not trying to add new mechanics<br>
&lt;TabAtkins> lea: speaking of needing more examples, I posted one recently in this thread where every UA gets it wrong today<br>
&lt;ChrisL> https://codepen.io/leaverou/pen/XJpybdZ?editors=1100<br>
&lt;TabAtkins> lea: if you have oklch color with nones, and you have oklch relative color that uses l/c/h, all browsers preserve one level of this, but at two or more they convert to 0<br>
&lt;TabAtkins> ChrisL: all three of these should be identical, they're just doing it with more indirection<br>
&lt;TabAtkins> romain: I think this is because a single sentence is hidden behind some examples<br>
&lt;TabAtkins> romain: I think impls are interpreting "none behaves as zero" as meaning it *produces* zero in relative colors. but that was clarified in the spec a while ago<br>
&lt;TabAtkins> romain: looking at current impls is tricky because they're all behind<br>
&lt;TabAtkins> romain: but this thread was about splitting one color conversation Algo into two parts, but nothing references those two parts individually<br>
&lt;TabAtkins> ChrisL: yes, they used to, but not anymore<br>
&lt;TabAtkins> romain: ok, that sent me down some rabbit holes<br>
&lt;ChrisL> q+<br>
&lt;TabAtkins> romain: so that's where i'm stuck at<br>
&lt;astearns> ack ChrisL<br>
&lt;TabAtkins> ChrisL: i'm a bit confused, we've kicked around this text for such a long time, I find it hard to step back and say what exactly we're doing and why<br>
&lt;TabAtkins> ChrisL: every individual change probably made sense but it's not clear to me it's doing the right thing overall<br>
&lt;TabAtkins> romain: yes i've had that problem too, implementing a change for one resolution makes things fall apart in other places<br>
&lt;TabAtkins> romain: it might make sense to share my impl and you can tell me what's wrong, it's matching latest spec afaik<br>
&lt;TabAtkins> lea: we do still need to update the spec too, last I looked it was very confusing<br>
&lt;TabAtkins> romain: editorially?<br>
&lt;TabAtkins> lea: yes<br>
&lt;TabAtkins> astearns: not knowing these particular details, have three thoughts<br>
&lt;TabAtkins> astearns: first, clarifying the spec<br>
&lt;romain> q+<br>
&lt;TabAtkins> astearns: second, whether the current impl differences are merely because they're behind, or if they're *allowed* to differ in the spec right now and we might need changes to make things clearer<br>
&lt;TabAtkins> astearns: third, if there are unobservable results from portions of the Algo that tests can't observe, maybe we can add an extension to webdriver to let us check those if it's important<br>
&lt;TabAtkins> ChrisL: that might be good.<br>
&lt;TabAtkins> ChrisL: having an impl that does what the espec says and we can use to verify examples will be useful and let us make sure the spec is clear<br>
&lt;TabAtkins> ChrisL: issue with browsers is they can easily be out of date<br>
&lt;TabAtkins> ChrisL: main problem is coercing hue to 0 when it's none, that usually makes things red/magenta<br>
&lt;TabAtkins> ChrisL: that makes people complain "why is this pink all of a sudden?" in gradients, it's from coercing to zero.<br>
&lt;astearns> ack romain<br>
&lt;TabAtkins> romain: I think for observability, there's one issue around infinity we're not sure how to test<br>
&lt;ydaniv> s/coercing to zero/coercing hue to zero/<br>
&lt;TabAtkins> romain: main problem ith wpt is you need so many tests<br>
&lt;TabAtkins> romain: updating is a big pain<br>
&lt;TabAtkins> romain: I created a script for this since my impl is in js, so I can just use a regex to replace results<br>
&lt;TabAtkins> romain: but yeah you just need a massive number of tests and you lack so many<br>
&lt;TabAtkins> ChrisL: if there's a script that generates it means we can *re*generate easily<br>
&lt;TabAtkins> astearns: so we'll take back to the issue. Romain will share impl and we'll look at making the spec clearer<br>
&lt;TabAtkins> romain: perfect<br>
</details>

<!-- comment id=5338496724 author=romainmenke created=2026-08-19T06:41:37Z url=https://github.com/w3c/csswg-drafts/issues/10211#issuecomment-5338496724 -->
## @romainmenke — 2026-08-19

My implementation can be found here:
https://github.com/csstools/postcss-plugins/tree/main/packages/css-color-parser

The script to update the WPT computed value tests: https://github.com/csstools/postcss-plugins/blob/main/packages/css-color-parser/test/util/update-wpt.mjs

I've also created a little playground that is web hosted and can be used to check individual values: https://preset-env.cssdb.org/color/computed/#JTdCJTIyY2klMjIlM0ElMjJjb2xvci1taXgoaW4lMjBoc2wlMkMlMjBoc2woMzBkZWclMjA0MCUyNSUyMDgwJTI1JTIwJTJGJTIwMjUlMjUpJTIwMCUyNSUyQyUyMGhzbCg5MGRlZyUyMG5vbmUlMjBub25lJTIwJTJGJTIwbm9uZSkpJTIyJTdE

Most relevant to this thread are these tests around `none` and powerless components:
- https://github.com/csstools/postcss-plugins/blob/main/packages/css-color-parser/test/basic/none-exhaustive.mjs
- https://github.com/csstools/postcss-plugins/blob/main/packages/css-color-parser/test/basic/powerless-exhaustive.mjs

<!-- comment id=5453488806 author=romainmenke created=2026-08-28T14:05:20Z url=https://github.com/w3c/csswg-drafts/issues/10211#issuecomment-5453488806 -->
## @romainmenke — 2026-08-28

I've now also submitted tests in WPT related to this issue: https://github.com/web-platform-tests/wpt/pull/62272

<!-- comment id=5652978540 author=svgeesus created=2026-09-13T11:28:39Z url=https://github.com/w3c/csswg-drafts/issues/10211#issuecomment-5652978540 -->
## @svgeesus — 2026-09-13

Related, a proposed Interop 2027 focus area:

 - https://github.com/web-platform-tests/interop/issues/1334
