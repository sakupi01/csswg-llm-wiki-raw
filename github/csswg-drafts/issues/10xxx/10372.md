...
state: "open"
labels: ["Needs Edits", "Needs Feedback/Review", "Needs Testcase (WPT)", "css-color-4"]
author: "josepharhar"
...
<!-- comment id=5202692386 author=css-meeting-bot created=2026-08-06T09:14:07Z url=https://github.com/w3c/csswg-drafts/issues/10372#issuecomment-5202692386 resolution=true -->
## @css-meeting-bot — 2026-08-06

The CSS Working Group just discussed `[css-color] Mitigating fingerprinting for AccentColor/AccentColorText`, and agreed to the following:
...
<details><summary>The full IRC log of that discussion</summary>
&lt;TabAtkins> tuankiet65: accentcolor and accentcolortext return the current color used from the OS level<br>
&lt;TabAtkins> tuankiet65: some OSes let you change that color, like MacOS and windows<br>
&lt;TabAtkins> tuankiet65: on Mac there are presets, on windows there's a color picker with free choice<br>
&lt;TabAtkins> tuankiet65: so concern is exposing it to the document is a fingerprinting vector<br>
&lt;TabAtkins> tuankiet65: a user might have a unique color<br>
&lt;TabAtkins> tuankiet65: WebKit returns a static color, regardless of user's choice<br>
&lt;TabAtkins> tuankiet65: I think that's what Chrome does too?<br>
&lt;TabAtkins> tuankiet65: if the page is installed chrome can return the real color<br>
&lt;TabAtkins> tuankiet65: I think that's from google's privacy team also thinking this is a fingerprinting vector<br>
&lt;TabAtkins> tuankiet65: the current spec says UA has discretion to return the actual color or a hard coded value<br>
&lt;TabAtkins> tuankiet65: are we okay with that? if it returns a hard coded color it's kinda useless<br>
&lt;TabAtkins> tuankiet65: but if it returns the actual color, is there a way to prevent the page from inferring the actual color?<br>
&lt;TabAtkins> tuankiet65: there was some discussion about making the color unobservable<br>
&lt;TabAtkins> tuankiet65: for example by preventing the color from being used in canvas, or marking it as tainted<br>
&lt;TabAtkins> tuankiet65: there are some other solutions we could explore<br>
&lt;TabAtkins> ntim: getComputedStyle() exposes it too<br>
&lt;emilio> q+<br>
&lt;TabAtkins> emilio: i'm fine with saying the UA can just lie<br>
&lt;TabAtkins> emilio: we've done that for all the other system colors<br>
&lt;astearns> ack emilio<br>
&lt;TabAtkins> emilio: the tainting stuff isn't conving to me<br>
&lt;lea> q+<br>
&lt;TabAtkins> emilio: it sounds really complicated to spec and implement to not leak it<br>
&lt;TabAtkins> emilio: for relatively little benefit<br>
&lt;TabAtkins> emilio: maybe a better approach is... I think we already resolved accentcolor reflects the accent-color property?<br>
&lt;TabAtkins> emilio: so via that resolution, accent-color is already not computed-value time, it's kept like currentcolor<br>
&lt;TabAtkins> fantasai: I think it still resolves at computed value time<br>
&lt;TabAtkins> emilio: if it depends on accent-color the property, you need to delay it, right?<br>
&lt;TabAtkins> emilio: you could resolve it eagerly, I suppose, like em units<br>
&lt;TabAtkins> emilio: but I guess we didn't define the exact resolution mechanism<br>
&lt;TabAtkins> emilio: if we did that it's not a regular system color, it's just a magic keyword<br>
&lt;TabAtkins> fantasai: I think it's kinda  a q about use-cases and impl complexity<br>
&lt;TabAtkins> fantasai: don't think we talked about it much as an issue<br>
&lt;TabAtkins> fantasai: currentcolor does need to reflect the current color even when inherited<br>
&lt;TabAtkins> fantasai: but accentcolor isn't set for text and inherited, really<br>
&lt;TabAtkins> fantasai: that's why currentcolor needs to compute to itself, but accentcolor doesn't really<br>
&lt;TabAtkins> fantasai: so having it eagerly compute like em units is fine, I think, from the use-cases<br>
&lt;TabAtkins> fantasai: and then it's much easier to manage for the browser<br>
&lt;kizu> q+<br>
&lt;TabAtkins> fantasai: we refused to do this for canvas/canvastext<br>
&lt;TabAtkins> fantasai: it doens't flip with color-scheme<br>
&lt;TabAtkins> fantasai: if we're happy to add more suspended used-value colors we can do it for those. accentcolor isn't as important<br>
&lt;TabAtkins> emilio: I brought this up because if we keep it around as a keyword it's easier to say gCS() doesn't resolve the keyword<br>
&lt;TabAtkins> emilio: much easier than tainting<br>
&lt;TabAtkins> emilio: I personally wouldnt' go that route...<br>
&lt;TabAtkins> emilio: but if we're going to hide the accent color, keeping it computed to itself does simplify a lot<br>
&lt;astearns> ack lea<br>
&lt;TabAtkins> lea: i'm not super convinced that this is that big of a fingerprint vector, but not my specialty<br>
&lt;TabAtkins> lea: I think accentcolor that UAs actually use should be the same color authors have access to, otherwise you get mismatched UIs<br>
&lt;emilio> +1<br>
&lt;TabAtkins> lea: remind the group we have resolution to allow extracting components<br>
&lt;TabAtkins> lea: so without JS you can still read components<br>
&lt;TabAtkins> lea: even if we reversed that resolution you can exfil color components anyway via comparison<br>
&lt;astearns> ack kizu<br>
&lt;kizu> https://kizu.dev/svg-linked-parameters-workaround/#querrying-the-actual-color<br>
&lt;TabAtkins> kizu: I have an example<br>
&lt;TabAtkins> kizu: it works for anything besides currentcolor<br>
&lt;TabAtkins> kizu: once a color in a custom property you can read the components out and assign them to other custom props<br>
&lt;TabAtkins> kizu: you can't do it with a used value like currentcolor<br>
&lt;TabAtkins> kizu: component querying functions will have the same problem if we define it to not work for currentcolor, etc<br>
&lt;emilio> q+<br>
&lt;TabAtkins> kizu: maybe we could allow currentcolor to work but not accentcolor, etc<br>
&lt;astearns> ack emilio<br>
&lt;TabAtkins> emilio: my preference would be no change<br>
&lt;TabAtkins> emilio: agree with Lea that UA usage and author usage should be identical<br>
&lt;TabAtkins> emilio: if we want to hide it and use the right accentcolor at rendering time, I think we do need to keep accentcolor as a used value , and accentcolortext<br>
&lt;TabAtkins> emilio: currently system colors compute to an actual color<br>
&lt;TabAtkins> astearns: but you said no spec change?<br>
&lt;TabAtkins> emilio: yes I think so. if the concern is fingerprinting we can just use a fixed color<br>
&lt;TabAtkins> emilio: Firefox does that, always blue, because people line up their OS accent colors to their backgrounds, etc<br>
&lt;TabAtkins> emilio: and I think that's ok so long as your native form controls are also blue<br>
&lt;TabAtkins> emilio: I think webkit does it too<br>
&lt;TabAtkins> emilio: I think tainting is a nightmare to spec and implement<br>
&lt;TabAtkins> astearns: so I think I heard three options<br>
&lt;TabAtkins> astearns: 1) have accentcolor compute to itself<br>
&lt;TabAtkins> emilio: and more changes to make sure gCS() returns computed value, etc<br>
&lt;TabAtkins> astearns: yes, each of these has attendant details<br>
&lt;TabAtkins> astearns: 2) have accentcolor compute to a fixed color<br>
&lt;TabAtkins> astearns: 3) taint it up<br>
&lt;TabAtkins> ntim: option 1 also needs tainting<br>
&lt;TabAtkins> emilio: we could do that with a fixed color into canvas, we just don't want to propagate taint across variables, etc<br>
&lt;TabAtkins> ntim: yeah also anything with accentcolor as an argument, like color-mix()...<br>
&lt;TabAtkins> fantasai: right he's saying *because* it's preserved as a used value, you know that the value contains accentcolor. no need for an extra taint bit<br>
&lt;TabAtkins> ntim: my pref is 2<br>
&lt;kbabbitt> q+<br>
&lt;sgill> q+<br>
&lt;TabAtkins> tuankiet65: I would prefer that we do no change, allow UA to return a fixed color fi they want<br>
&lt;TabAtkins> tuankiet65: tainting thing, there's a lot of places you have to pay attention to, and you can easily miss a place and expose it<br>
&lt;astearns> ack kbabbitt<br>
&lt;TabAtkins> kbabbitt: just to clarify, pref is that UAs *may* return a fixed color? or must?<br>
&lt;fantasai> scribe+<br>
&lt;fantasai> fantasai: Should<br>
&lt;TabAtkins> TabAtkins: allowing UAs to return the real color in things like installed apps<br>
&lt;emilio> sgtm<br>
&lt;astearns> ack sgill<br>
&lt;TabAtkins> ntim: make it a should, with a note saying UAs can return real color where fingerprinting isn't a concern<br>
&lt;TabAtkins> sgill: it's mentioned you can extract component values<br>
&lt;fantasai> I would not make that a note, should be normative<br>
&lt;fantasai> :)<br>
&lt;TabAtkins> sgill: if we went with 1, is there still concern about that approach?<br>
&lt;TabAtkins> emilio: 1 would let us mitigate that more straightforward<br>
&lt;TabAtkins> emilio: you still need to figure out places where you do resolve the colors, but that's an easier list to find<br>
&lt;TabAtkins> emilio: color extraction just wouldn't work, samea s for currentcolor now<br>
&lt;TabAtkins> emilio: then once you're past computed value you know if the keyword is around<br>
&lt;TabAtkins> emilio: canvas could paint with a fixed color, etc<br>
&lt;TabAtkins> lea: what about if()?<br>
&lt;TabAtkins> emilio: same as currentcolor, the value you compare against will be the keyword<br>
&lt;TabAtkins> sgill: would it still be possible to accidentally leak?<br>
&lt;TabAtkins> emilio: yes, still whack-a-mole, but much easier<br>
&lt;lea> q?<br>
&lt;lea> q+<br>
&lt;emilio> q+<br>
&lt;TabAtkins> astearns: proposed: accentcolor and accentcolortext, UAs *should* return (and use) a fixed color as the initial (os-dependent) value, can expose real value in places where fingerprinting isn'ta  cocnern<br>
&lt;astearns> ack lea<br>
&lt;TabAtkins> lea: not opposed, but seems extreme complexity<br>
&lt;fantasai> lea: nevermind, +1<br>
&lt;TabAtkins> lea: [never mind was confused]<br>
&lt;TabAtkins> astearns: objections?<br>
&lt;kbabbitt> +1<br>
&lt;lea> +1<br>
&lt;tuankiet65> *may* expose real value I suppose, but +1<br>
&lt;TabAtkins> RESOLVED: accentcolor and accentcolortext, UAs *should* return (and use) a fixed color as the initial (os-dependent) value, can expose real value in places where fingerprinting isn'ta  cocnern<br>
&lt;ChrisL> q+<br>
&lt;astearns> ack emilio<br>
&lt;TabAtkins> emilio: follow up, need a resolution for how the accent-color property lookup works<br>
&lt;TabAtkins> emilio: whether it's looked up like "em" or like "currentcolor"<br>
&lt;astearns> ack ChrisL<br>
&lt;TabAtkins> emilio: if we do it like em we need to define its behavior in 'accent-color' itself<br>
&lt;TabAtkins> fantasai: that's the next issue<br>
&lt;TabAtkins> ChrisL: if we put this in the spec privacy people might get weird about it, do we have good wording?<br>
&lt;TabAtkins> TabAtkins: "UAs for which fingerprinting isnt' a concern" is a valid class, installed web apps<br>
&lt;TabAtkins> fantasai: or PrinceXML, printing doesn't care<br>
</details>

<!-- comment id=5205842165 author=phaux created=2026-08-06T14:13:37Z url=https://github.com/w3c/csswg-drafts/issues/10372#issuecomment-5205842165 -->
## @phaux — 2026-08-06

Maybe instead of returning a hard-coded value, at least take the real accent color and "round" it to the nearest color from a set of predefined allowed accent color values. For example few saturated colors with different hues and 1 neutral color.

This way there will be no way to detect a person based on their accent color alone, because many users will share the same color, and a website will still be able to match the user color preference roughly if it wants, although not exactly.
