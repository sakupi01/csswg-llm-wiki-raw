...
<!-- comment id=4886096163 author=emilio created=2026-07-05T12:49:37Z url=https://github.com/w3c/csswg-drafts/issues/10002#issuecomment-4886096163 -->
## @emilio — 2026-07-05

This is not really easily doable without a lot more bookkeeping (and associated performance or memory usage cost). Currently, computed styles only store effectively what's exposed on typed OM.

There's also different "stages" of what you may want (variables substituted vs. not, units like vh computed vs not, css-wide keywords vs not...). E.g. if the specified value is `revert` / `revert-layer`, you presumably would want the reverted-to value? Otherwise there's no way of doing anything useful with it.

This would also expose the internal values that UAs use which we'd want to avoid, e.g. the internal base appearance stuff, or the quirky em units that webkit has...
