...
<!-- comment id=5827717282 author=kojiishi created=2026-09-25T06:09:14Z url=https://github.com/w3c/csswg-drafts/issues/10850#issuecomment-5827717282 -->
## @kojiishi — 2026-09-25

I agree with @jfkthame that predictability is important.

Another important factor of `text-box-trim` is accuracy, because the feature is for when authors want to very precisely align objects. For that purpose, I agree with @jfkthame that computing bounding isn't really a good fit.

How about not trimming when the font doesn't have the metadata? This is the same behavior as when applying missing font features. It's easy to find it's not applied. It's predictable even for subset fonts. Authors could use JS to determine when it's not applied.

<!-- comment id=5831444168 author=jfkthame created=2026-09-25T11:13:53Z url=https://github.com/w3c/csswg-drafts/issues/10850#issuecomment-5831444168 -->
## @jfkthame — 2026-09-25

I don't think we've come up with a clear response to this point from the JLReq TF Meeting feedback:

> It goes without saying that these values must be calculated based on the first CJK font in the font list because the first font in the list is typically chosen based on the author’s intent for Latin glyphs rather than CJK text.

One concern I have here is that it may not be clear what "the first CJK font in the font list" actually is. Are there unambiguous criteria that identify a font as being "a CJK font" vs "not a CJK font"? If it's something about character set coverage, will there be a risk that subsetting a font changes its "CJK-ness" when the author may not expect this?
