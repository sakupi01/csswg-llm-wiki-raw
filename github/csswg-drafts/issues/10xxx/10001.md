...
This issue is now basically asking for a shortcut `.foo-*` for existing `[class|=foo]` **except** that pipe matching only works for the first entry of a space separated list of values, i.e. `<a class="bar foo-quuz">` would not be matched but `<a class="foo-quuz bar">` and `<a class="foo foo-quuz">` would. 
