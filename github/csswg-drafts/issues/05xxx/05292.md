...
<!-- comment id=5440519820 author=Loirooriol created=2026-08-27T14:22:04Z url=https://github.com/w3c/csswg-drafts/issues/5292#issuecomment-5440519820 -->
## @Loirooriol — 2026-08-27

About compositing with the grandparent, I also don't like that if that also has a semitransparent color, the algorithm may need to keep compositing up to the root. And it just seems too fragile in general.

@Crissov I don't think `Current` would be clear enough, I think it would just bring confusion.

<!-- comment id=5440633684 author=flackr created=2026-08-27T14:31:01Z url=https://github.com/w3c/csswg-drafts/issues/5292#issuecomment-5440633684 -->
## @flackr — 2026-08-27

> About compositing with the grandparent, I also don't like that if that also has a semitransparent color, the algorithm may need to keep compositing up to the root. And it just seems too fragile in general.

It just needs to composite on top of the parent's `current-background-color` which has already accounted for compositing on top of its parent. This is I think pretty simple to compute as an internal style property that has a bit of inheriting logic to composite its own background color on top of the inherited value.

Can you expand on what you mean by this is too fragile?

Also, if we don't do this, are you proposing we just use the new partially transparent color as if it is the current background color? I think this will lead to many sites getting non-contrasting colors. E.g.
```css
.item {
  color: contrast-color(currentBackgroundColor);
}

.item:hover {
  background: rgba(0, 0, 0, 0.1); /* slight shade on highlighted item */
}
```

While hovering, the text would then assume it needs to constrast with `rgba(0, 0, 0, 0.1)`? Rather than the effective background of roughly rgb(230, 230, 230)?

<!-- comment id=5440770456 author=Loirooriol created=2026-08-27T14:41:47Z url=https://github.com/w3c/csswg-drafts/issues/5292#issuecomment-5440770456 -->
## @Loirooriol — 2026-08-27

> It just needs to composite on top of the parent's current-background-color

Fair

> Can you expand on what you mean by this is too fragile?

Maybe it wasn't the right word. I mean that it seems very likely to break in complex cases, while seeming good in simpler cases. So I think it can end up misleading authors further than just not compositing.

In other words, while compositing may make a few cases look better, I think that there will still be too many broken cases, and the confusion isn't really worth.

> are you proposing we just use the new partially transparent color as if it is the current background color?

Well, partially, but I don't like that either. If anything, I think I would prefer the `accent-color` approach.

<!-- comment id=5441280602 author=flackr created=2026-08-27T15:19:43Z url=https://github.com/w3c/csswg-drafts/issues/5292#issuecomment-5441280602 -->
## @flackr — 2026-08-27

> If anything, I think I would prefer the accent-color approach.

Right, personally I like calling this `canvas-color` as suggested in https://github.com/w3c/csswg-drafts/issues/5292#issuecomment-5344945094.

That said, we could try to do both, e.g. `magic-background-color: auto` would composite the local background color on top of the ancestor's canvas color. Then in any context we're you're positioning the element out of flow the best practice would be to set the `magic-background-color` to what you think is a good representative color of what the element is on top of. I intentionally used a silly name so we can focus on the idea rather than the name itself.

<!-- comment id=5494180737 author=LeaVerou created=2026-09-01T12:48:27Z url=https://github.com/w3c/csswg-drafts/issues/5292#issuecomment-5494180737 -->
## @LeaVerou — 2026-09-01

> Right, personally I like calling this `canvas-color` as suggested in [(earlier comment)](https://github.com/w3c/csswg-drafts/issues/5292#issuecomment-5344945094).

Actually, we already have `canvas` as a system color. What if we redefined it to work like `currentBackgroundColor` except when used in `background-color`, where it would be what it is today. That's consistent with how `AccentColor` is specced to work, and much more elegant than a 3-word concatenatedcase keyword. It may not be web-compatible though (that's easy to figure out), in which case maybe `currentcanvas`?

<!-- comment id=5546126065 author=phaux created=2026-09-04T20:32:47Z url=https://github.com/w3c/csswg-drafts/issues/5292#issuecomment-5546126065 -->
## @phaux — 2026-09-04

@LeaVerou `canvas-color` would be the property which sets the `Canvas` color, like `accent-color` sets the `AccentColor`. Elements which want to have a matching background would use `background-color: Canvas`. You would set `canvas-color: YourColor` on `:root` and on every container element which has a different background and has children which need to match it.
