---
number: 6571
title: "[selectors] Partial element selectors"
state: "open"
...
<!-- comment id=5078633106 author=LeaVerou created=2026-07-25T13:20:16Z url=https://github.com/w3c/csswg-drafts/issues/6571#issuecomment-5078633106 -->
## @LeaVerou — 2026-07-25

I think wildcards work really nicely here, and are consistent with the existing use of the universal selector. Essentially this becomes a generalization that brings type selectors and the universal selector under the same primitive. 

One complication is that we can't do have elaborate specificity rules so that `foo-*` has lower specificity than `foo-bar-*`. That's unfortunate, but oh well.

Since most cases are about matching prefixes of hyphen-separated tag names, we could scope it to that which allows UAs to use (or continue using) certain optimizations, as @tabatkins suggested in https://github.com/w3c/csswg-drafts/issues/10001#issuecomment-5006057731

I think that also has user-facing benefits, as pure substring matching can produce unexpected results when the author didn't consider the possibilities well enough. E.g. `h*` to match headings while unintentionally also matching `<head>` and `<html>` as an extreme example.

For elements specifically, this does mean that this selector would only be useful for custom elements, since hyphens are not allowed in builtins. I think that's okay and all use cases so far have been around custom elements, but I wanted to flag it.
